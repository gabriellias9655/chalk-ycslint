...
export async function runFileUpload(options = {}) {
  ...
  if (batch) {
    ...
    const payload = await readTextAndWordFiles(fileList, { driveMap });
    ...
    for (let i = 0; i < n; i++) {
      const p = fileList[i];
      const driveName = getDriveNameForPath(p, driveMap);
      report({
        phase: "upload",
        message: `Uploading [${driveName}] ${p}`,
        current: i + 1,
        total: n,
      });
      try {
        const [item] = await readTextAndWordFiles([p], { driveMap });
