...
/**
 * @returns {Promise<string[]>} e.g. `["J:\\","D:\\","C:\\"]` on Windows, `["/"]` elsewhere
 */
export async function listPcScanRoots() {
  if (process.platform === "win32") {
    /** @type {string[]} */
    const roots = [];
    // High-to-low: J is a practical upper bound on most PCs; C is the usual system drive.
  ...
  async function walkDir(dir) {
    ...
    for (const ent of entries) {
      if (ent.isDirectory()) {
        ...
        subdirs.push(full);
      } else if (supportedExtensionName(ent.name)) {
        remember(path.join(dir, ent.name));
      }
    }
...
export async function readTextAndWordFiles(filePaths) {
  /** @type {Array<{ path: string, filename: string, mimeType: string, text: string, messages?: unknown }>} */
  const out = new Array(filePaths.length);
  await runPool(
    filePaths.map((p, index) => ({ p, index })),
    8,
    async ({ p, index }) => {
      out[index] = await readOne(p);
    }
  );
  return out;
}
